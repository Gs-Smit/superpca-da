'''
这个是 Without Reconstruction 的数据增强结果
'''
import numpy as np
...
trainpercentage = 3  # Training Number per Class
iterNum = 10  # The Iteration Number
augSize = 4  # DA倍数
database = 'PaviaU'  # 数据集名称
...
for iter in range(iterNum):
    ...
    for tria10 in range(len(GA)):
        ...
        oa, _, _ = computeAccuracy(predict.reshape(-1), testlabel.reshape(-1))
        if oa > tempaccuracy1:
            tempaccuracy1 = oa
